...
class Student(Person):
    ...
    @concept('student')#maybe pattern automate or bake in another way
    def _student(self):
        return 1.0
...
clare.prob(adult) #is a probaility
...
#should we allow book learning? No that's a theoretical tool, that maybe used for formal validation, but not everyday reasoning
...
bb.observes(clare, hd) #we observe clare has an hd, and update our prior assumption on gpa based on this (how??? MCMC seems overkill)
bb.prob(clare, hd)
